module CommObs
  ...
  def obs_the_comm
    puts 'obs_the_comm(answer) obs_the_comm(answer) obs_the_comm(answer)'
    puts self
    after_comment_create(self)
    puts '+   +   +   +   +   class   +   +   +   +   +'
    puts self.class
    puts self.inspect
    puts '+   +   +   +   +   +    +   +   +   +   +   +'
  ...
  def after_comment_create(comment_join_object)
    ua = get_user_array(comment_join_object)
    puts 'ua: ' + ua.inspect
    ua.each do |u_a|
      puts 'u_a ' + u_a
      # CommentMailer.also_comment_mail(comment_join_object, u_a).deliver_later
      MailAfterAlsoCommentJob.set(wait: 1.minute).perform_later(comment_join_object, u_a)
    end
    ...
    if comment_join_object.is_a?(Answer)
      if !ua.include?(comment_join_object.micropost.user.email) && comment_join_object.comment.user.email != comment_join_object.micropost.user.email
        ...
        if en.nil? || en.emails == true
          ...
          MailAfterCommentJob.set(wait: 1.minute).perform_later(comment_join_object)
        end
      end
